...
const onChange = (file: UploadFiles, filelist: UploadFiles[]) => {
    ...
    if (props.options.fileType.length > 0 && props.options.fileType.indexOf(type) == -1)
        return ElMessage.warning(`请上传${props.options.fileType.join(',')}格式的文件`);

    filelist.splice(index + 1, 0, file);
    return emits('change', file, filelist);
};

const beforeRemove = (file: UploadFiles, filelist: UploadFiles[]): Promise<boolean> => {
